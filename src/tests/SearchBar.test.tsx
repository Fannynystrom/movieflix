...
const mockData = [
  { title: "Movie 1", synopsis: "Synopsis 1", genre: "Genre 1", actors: ["Actor 1"] },
  { title: "Movie 2", synopsis: "Synopsis 2", genre: "Genre 2", actors: ["Actor 2"] },
];
...
describe("SearchBar Component", () => {
  ...
  test("toggles the search field visibility when icon is clicked", () => {
    ...
    expect(screen.getByPlaceholderText("Sök filmer...").closest('div')).toHaveClass('active');
    fireEvent.click(searchIcon);
    expect(screen.getByPlaceholderText("Sök filmer...").closest('div')).not.toHaveClass('active');
  });
  ...
  test("displays up to 8 search results", () => {
    const extendedData = [...Array(10).keys()].map(i => ({ title: `Movie ${i + 1}` }));
